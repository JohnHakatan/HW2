...
public:
    ...
    void setRankAsLeaf(int grade);
    void setRank(int rank);
    void setSub_tree_size(int size);
    void updatSubtreeSize();
    void updateHeight();
...
template <class T, class comparator>
void AVLNode<T, comparator>::setRank(int rank)
{
    this->Rank = rank;
    // calling update rank for others
}
template <class T, class comparator>
void AVLNode<T, comparator>::setSub_tree_size(int size)
{
    this->sub_tree_size = size;
}
template <class T, class comparator>
void AVLNode<T, comparator>::updateHeight()
{
    if (this != nullptr)
    {
        int val = 1;
...
template <class T, class comparator>
void AVLNode<T, comparator>::updatSubtreeSize()
{
    ...
    if (this != nullptr)
    {
        ...
        if (this->left != nullptr)
        {
            val += this->left->sub_tree_size;
        }
        if (this->right != nullptr)
        {
            val += this->right->sub_tree_size;
        }
        this->setSub_tree_size(val);
    }
}
template <class T, class comparator>
void AVLNode<T, comparator>::setData(const T data)
{
    this->data = data;
}
...
template <class T, class comparator>
void AVLNode<T, comparator>::updateRank()
{
    if(this!=nullptr){
    int left_son_rank = 0;
    int right_son_rank = 0;
    if (this->getLeft())
    {
        left_son_rank = this->getLeft()->getRank();
    }
    if (this->getRight())
    {
...
public:
    ...
    int AVLInsert(const T to_add , int grade);
...
bool AVLTree<T, comparator>::moveTree(AVLTree<T, comparator> &moved, AVLTree<T, comparator> &to)
{
    ...
    {
        if (to.AVLInsert((ptr->getData()),ptr->getRankAsLeaf()) == MEMORY_ERROR)
        {
...
AVLNode<T, comparator> *AVLTree<T, comparator>::leftLeftRotation(AVLNode<T, comparator> *unbalanced)
{
    ...
    if (parent->getRight() == unbalanced)
        right = true;
    if (parent->getLeft() == unbalanced)
        left = true;
    ...
    //option for assert - to check if B-rank before rotation= A rank after rotation
    unbalanced->updateRank();
    unbalanced->updatSubtreeSize();
    new_root->height = max(new_root->getRight()->getHeight(), new_root->getLeft()->getHeight()) + 1;
    new_root->updateRank();
    new_root->updatSubtreeSize();
    return new_root;
}
...
int AVLTree<T, comparator>::balanceFactor(const AVLNode<T, comparator> *root1) const
{
    ...
    if (!root1->getLeft())
        return 0 - root1->getRight()->getHeight();
    if (!root1->getRight())
        return root1->getLeft()->getHeight();
...
void AVLTree<T, comparator>::AVLInsertTheNode(AVLNode<T, comparator> *to_add)
{
    ...
    if (parent)
        update_and_check_heights(parent);
...
template <class T, class comparator>
int AVLTree<T, comparator>::AVLInsert(const T to_add,int grade)
{
    if (find(root, to_add))
    {
        return DATA_EXIST;
    ...
    try
    {
        new_node = new AVLNode<T, comparator>(to_add,grade);
    }
    ...
    if (cmp(to_add, (this->maxNode)->getData()) > 0)
        this->maxNode = new_node;
...
AVLNode<T, comparator> *AVLTree<T, comparator>::AVLRemove(AVLNode<T, comparator> *iter, const T to_del)
{
    ...
    { // this is the node to remove.
        ...
        if (!iter->right && !iter->left)
        {
            if (iter != this->root)
            {
                if (iter == iter->getParent()->getRight())
                    iter->getParent()->setRight(nullptr);
                if (iter == iter->getParent()->getLeft())
                    iter->getParent()->setLeft(nullptr);
            }
            delete iter;
            this->num_of_nodes--;
            iter = nullptr;
...
AVLNode<T, comparator> *AVLTree<T, comparator>::find_max_min_than(AVLNode<T, comparator> *root1, const T &data) // not checked
{
    ...
    if (!root1->getLeft() && cmp(tmp, data) <= 0)
        return root1;
    if (cmp(tmp, data) == 0)
    {
        return root1;
